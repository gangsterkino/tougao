...
import './index.less';
import { startTrainingModel3B } from '@/api/train'; // 导入模型2的训练接口
import { getModel3TrainingStatus, getModel3FaultData } from '@/api/getData'; // 导入获取训练状态和故障数据的接口
...
const Sensitive: React.FC = () => {
    ...
    const startTraining = () => {
        ...
        // 调用训练接口
        startTrainingModel3B({ learningRate, epochs })
            .then((response) => {
    ...
    const startTest = () => {
        ...
        getModel3FaultData()
            .then((data) => {
                ...
                setFaultData(data);
                setFaultImage(`http://127.0.0.1:5000/${data['image_path']}`);
            })
            .catch((error) => {
